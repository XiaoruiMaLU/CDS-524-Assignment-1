...
class Game:
    def __init__(self, master, player1, player2, Q_learn=None, Q={}, alpha=0.3, gamma=0.9):
        ...
        # Set row and column weights to make sure the frame expands to fill the window
        ...
        # Stats label to display the results (wins, draws, win rate)
        self.stats_label = tk.Label(frame, text="Human: 0 (0%)  Computer: 0 (0%)  Draw: 0 (0%) Total: 0")
        self.stats_label.grid(row=4, columnspan=10)

         # Label to display the result (who won or if it's a draw)
        self.result_label = tk.Label(frame, text="", font=("Arial", 14))
        self.result_label.grid(row=5, columnspan=10)
    ...
    def callback(self, button):
        if self.board.over():
            ...
            if isinstance(self.current_player, HumanPlayer) and isinstance(self.other_player, HumanPlayer):
                if self.empty(button):#the button is not clicked
                    move = self.get_move(button)
                    self.handle_move(move)
            # If the current player is a human and the other player is a computer, the human player should make a move and then the computer player should make a move
    ...
    def update_stats(self):
        # Update stats on the UI label
        human_win_rate = (self.human_wins / self.total_games * 100) if self.total_games > 0 else 0
        computer_win_rate = (self.computer_wins / self.total_games * 100) if self.total_games > 0 else 0
        draw_rate = (self.draws / self.total_games * 100) if self.total_games > 0 else 0
        stats_text = f"Human: {self.human_wins} ({human_win_rate:.1f}%)  Computer: {self.computer_wins} ({computer_win_rate:.1f}%)  Draw: {self.draws} ({draw_rate:.1f}%) Total: {self.total_games}"
        self.stats_label.config(text=stats_text)  # Update the label with the latest stats
    ...
    def declare_outcome(self):
        ...
        if self.board.winner() is None:
            ...
            self.result_label.config(text=f"The player with {mark} wins!")
            if mark == "X":
                self.human_wins += 1
            else:
                self.computer_wins += 1
        self.total_games += 1  # Increment total games count
        self.update_stats()  # Update the stats display after each game
    ...
    def reset(self):
        ...
        self.result_label.config(text="")  # Clear result label when reset
    ...
    def learn_Q(self, move):                        # If Q-learning is toggled on, "learn_Q" should be called after receiving a move from an instance of Player and before implementing the move (using Board's "place_mark" method)
        ...
        print("next_board", next_board.grid)
        ...
        if next_board.over():
            expected = reward
            print("next board over expected", expected)
        else:
            ...
            if self.current_player.mark == "X":
                ...
                print("O expected", expected)
        change = self.alpha * (expected - self.Q[state_key][move])
        print("state: ",state_key,"action: ",move,"Q值更新前", self.Q[state_key][move])
        self.Q[state_key][move] += change
        print("Q值更新后", self.Q[state_key][move])
...
class Board:
    ...
    def over(self):             # over condition: The game is over if there is a winner or if no squares remain empty (cat's game)
    ...
    def make_key(self, mark):          # For Q-learning, returns a 10-character string representing the state of the board and the player whose turn it is
        fill_value = 9 # 9 is used to represent an empty square
        filled_grid = copy.deepcopy(self.grid)
        np.place(filled_grid, np.isnan(filled_grid), fill_value)# fill 9
